...
function capturarFilaCargos(id) {
    ...
    $('#' + id + ' tbody tr').click(function () {
        ...
        v_idpersona = $(this).find("td").eq(1).html();
        v_nroseguro = $(this).find("td").eq(2).html();
        v_nombre = $(this).find("td").eq(3).html();
        v_ci = $(this).find("td").eq(4).html();
        

        $('#txtId').val(v_id);
        $('#txtid_persona').val(v_idpersona);
        $('#txtnroseguro').val(v_nroseguro);
...
function cargarBuscadorC(opcion) {
    //alert(opcion);
    switch (opcion) {
        case 'persona' :
            casoActivo = 'persona';
...
function recuperarPersona_enterC(e) {
